...
/** Normalize API origin: keep host root even if env is `https://host/api`. */
...
/**
 * Socket.IO attaches to the HTTP server root — strip a trailing `/api` from REST base URL
 * so `ws://host/socket.io` matches when `VITE_BASE_URL` is `http://host/api`.
 */
