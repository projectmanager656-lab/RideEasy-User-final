...
const CaptainSignup = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if (bFilled > 0 && bFilled < 4) {
      alert('Bank details: enter holder, account, IFSC, and UPI together, or leave all blank.')
      return
    }
    try {
      setLoading(true)
    ...
    <div className="py-5 px-5 min-h-screen flex flex-col justify-between">
      <div>
        ...
        <form onSubmit={submitHandler} className="space-y-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                >
                  {p.label}
                  {p.recommended ? <div className="text-[10px] font-semibold text-emerald-600">Recommended</div> : null}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">UPI ID for payments (optional)</label>
            <input className="bg-[#eeeeee] rounded-lg px-4 py-2 border w-full text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" type="text" placeholder="you@paytm" value={upiId} onChange={(e) => setUpiId(e.target.value)} />
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
            <p className="text-sm font-medium text-slate-700">Bank details for payouts (optional)</p>
            <p className="text-xs text-slate-500">Fill all four or leave empty.</p>
            <input className="bg-white rounded-lg px-4 py-2 border w-full text-slate-900" placeholder="Account holder name" value={accountHolderName} onChange={(e) => setAccountHolderName(e.target.value)} />
            <input className="bg-white rounded-lg px-4 py-2 border w-full text-slate-900" placeholder="Account number" value={accountNumber} onChange={(e) => setAccountNumber(e.target.value.replace(/\D/g, ''))} inputMode="numeric" />
            <input className="bg-white rounded-lg px-4 py-2 border w-full text-slate-900 uppercase" placeholder="IFSC" value={ifscCode} onChange={(e) => setIfscCode(e.target.value.toUpperCase())} />
            <input className="bg-white rounded-lg px-4 py-2 border w-full text-slate-900" placeholder="UPI (can match above)" value={bankUpiId} onChange={(e) => setBankUpiId(e.target.value)} />
          </div>
          <div>
